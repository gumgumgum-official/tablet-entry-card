...
/** Chaikin 오픈 폴리라인 한 단계 */
function chaikinOnceOpen(points: SubmitPoint[]): SubmitPoint[] {
  const n = points.length;
  if (n <= 2) return points.map((p) => ({ ...p }));

  const out: SubmitPoint[] = [{ ...points[0]! }];
  for (let i = 0; i < n - 1; i++) {
    const p = points[i]!;
    const q = points[i + 1]!;
    out.push({
...
/**
 * 단일 스트로크 중심선 스무딩 (Chaikin)
 */
export function smoothStroke(
  points: SubmitPoint[],
  options: SmoothStrokeOptions = {}
): SubmitPoint[] {
  const iterations = options.chaikinIterations ?? 2;
...
export function smoothStrokes(
  ...
  return strokes.map((s) => smoothStroke(s, options));
}

type XY = { x: number; y: number };

/**
 * 폴리라인을 Quadratic Bezier `d` 문자열로 (svg-converter와 동일 규칙)
 */
